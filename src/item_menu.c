...
static void ItemMenu_RegisterList(u8);
...
static void Task_RechargeableRepelMenu(u8);
static void RechargeableRepelMenu_Print(u8);
static void RechargeableRepelMenu_Close(u8);
static void RechargeableRepelMenu_ShowMessage(u8, const u8 *);
static void Task_RechargeableRepelMenuMessage(u8);
...
static void ItemMenu_FinishRegister(u8);
static void ItemMenu_FailRegister(u8);
...
static const u8 sText_RechargeableRepelSteps[] = _("Pasos restantes: {STR_VAR_1}/1000");
static const u8 sText_RechargeableRepelRecharge[] = _("Recargar");
static const u8 sText_RechargeableRepelExit[] = _("Salir");
static const u8 sText_RechargeableRepelNoCharges[] = _("No te quedan Cargas de repelente.");
static const u8 sText_RechargeableRepelFull[] = _("El depósito ya está al máximo.");
static const u8 sText_RechargeableRepelCharged[] = _("Recargaste +100 pasos.");
...
static const u8 sFontColorTable[][3] = {
    ...
    [COLORID_BLACK]       = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY,  TEXT_COLOR_LIGHT_GRAY}
};
...
static const u8 sText_TooManyRegistered[] = _("You already have too\nmany items registered!");
...
{
    ItemMenu_FinishRegister(taskId);
    DisplayItemMessage(taskId, FONT_NORMAL, sText_TooManyRegistered, ItemMenu_Cancel);
}
...
bool8 UseRegisteredKeyItemOnField(u8 button)
{
    ...
    if (button >= 2 && button <= REGISTERED_ITEMS_MAX + 1)
...
void StartRechargeableRepelMenu(u8 taskId)
{
    u8 windowId = BagMenu_AddWindow(ITEMWIN_RECHARGEABLE_REPEL);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(1));
    RechargeableRepelMenu_Print(windowId);
    CopyWindowToVram(windowId, COPYWIN_FULL);
    InitMenuNormal(windowId, FONT_NORMAL, 0, 16, 16, 2, 0);
    gTasks[taskId].func = Task_RechargeableRepelMenu;
}

static void RechargeableRepelMenu_Print(u8 windowId)
{
    ConvertIntToDecimalStringN(gStringVar1, gSaveBlock1Ptr->rechargeableRepelSteps, STR_CONV_MODE_LEFT_ALIGN, 4);
    StringExpandPlaceholders(gStringVar4, sText_RechargeableRepelSteps);
    BagMenu_Print(windowId, FONT_NORMAL, gStringVar4, 0, 1, 0, 0, 0, COLORID_BLACK);
    BagMenu_Print(windowId, FONT_NORMAL, sText_RechargeableRepelRecharge, 12, 16, 0, 0, 0, COLORID_BLACK);
    BagMenu_Print(windowId, FONT_NORMAL, sText_RechargeableRepelExit, 12, 32, 0, 0, 0, COLORID_BLACK);
}

static void Task_RechargeableRepelMenu(u8 taskId)
{
    s8 input = Menu_ProcessInputNoWrap();
    switch (input)
    {
    case 0:
        if (gSaveBlock1Ptr->rechargeableRepelSteps >= RECHARGEABLE_REPEL_MAX_STEPS)
        {
            RechargeableRepelMenu_ShowMessage(taskId, sText_RechargeableRepelFull);
        }
        else if (!CheckBagHasItem(ITEM_CARGA_DE_REPELENTE, 1))
        {
            RechargeableRepelMenu_ShowMessage(taskId, sText_RechargeableRepelNoCharges);
        }
        else
        {
            u16 steps = gSaveBlock1Ptr->rechargeableRepelSteps + RECHARGEABLE_REPEL_CHARGE_STEPS;
            if (steps > RECHARGEABLE_REPEL_MAX_STEPS)
                steps = RECHARGEABLE_REPEL_MAX_STEPS;
            gSaveBlock1Ptr->rechargeableRepelSteps = steps;
            RemoveBagItem(ITEM_CARGA_DE_REPELENTE, 1);
            RechargeableRepelMenu_ShowMessage(taskId, sText_RechargeableRepelCharged);
        }
        break;
    case 1:
    case MENU_B_PRESSED:
        RechargeableRepelMenu_Close(taskId);
        break;
    }
}

static void RechargeableRepelMenu_Close(u8 taskId)
{
    BagMenu_RemoveWindow(ITEMWIN_RECHARGEABLE_REPEL);
    ReturnToItemList(taskId);
}

static void RechargeableRepelMenu_ShowMessage(u8 taskId, const u8 *msg)
{
    BagMenu_RemoveWindow(ITEMWIN_RECHARGEABLE_REPEL);
    DisplayItemMessage(taskId, FONT_NORMAL, msg, Task_RechargeableRepelMenuMessage);
}

static void Task_RechargeableRepelMenuMessage(u8 taskId)
{
    if (JOY_NEW(A_BUTTON))
    {
        PlaySE(SE_SELECT);
        RemoveItemMessageWindow(ITEMWIN_MESSAGE);
        StartRechargeableRepelMenu(taskId);
    }
}
