...
static u8 CheckForPlayerAvatarCollision(u8 direction)
{
    ...
    if (!TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING | PLAYER_AVATAR_FLAG_UNDERWATER)
     && MetatileBehavior_IsSurfableFishableWater(metatileBehavior))
...
static u8 CheckForPlayerAvatarStaticCollision(u8 direction)
{
    ...
    return CheckForObjectEventStaticCollision(playerObjEvent, x, y, direction, metatileBehavior);
}
...
u8 CheckForObjectEventCollision(struct ObjectEvent *objectEvent, s16 x, s16 y, u8 direction, u8 metatileBehavior)
{
    ...
    // While surfing, only water tiles are traversable.
    // Attempt to dismount on any non-water tile that is a valid landing spot.
    if (gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_SURFING)
    {
        if (!MetatileBehavior_IsSurfableWaterOrUnderwater(metatileBehavior))
        {
            if (CanStopSurfing(x, y, direction))
...
static bool8 CanStopSurfing(s16 x, s16 y, u8 direction)
{
    ...
    if ((gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_SURFING)
     ...
     || GetObjectEventIdByPosition(x, y, elevation) == GetFollowerNPCObjectId()
     ))
    {
...
bool8 IsPlayerFacingSurfableFishableWater(void)
{
    ...
    metatileBehavior = MapGridGetMetatileBehaviorAt(x, y);
    if (MetatileBehavior_IsSurfableFishableWater(metatileBehavior))
        return TRUE;
    else
        return FALSE;
}
