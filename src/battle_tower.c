...
static void Task_BattleFactoryStageScene(u8 taskId);

#define FACTORY_STAGE_PLAYER_LOCALID (OBJ_EVENT_ID_DYNAMIC_BASE + 1)
#define FACTORY_STAGE_OPP_LOCALID    (OBJ_EVENT_ID_DYNAMIC_BASE + 2)
#define FACTORY_STAGE_PAUSE_FRAMES 24

static EWRAM_DATA u8 sFactoryStagePlayerObjId;
static EWRAM_DATA u8 sFactoryStageOppObjId;
static EWRAM_DATA bool8 sFactoryPartyPrepared;
...
static u16 GetFactoryMegaStoneForSpecies(u16 species)
{
    ...
    for (u32 i = 0; formChanges[i].method != FORM_CHANGE_TERMINATOR; i++)
    {
        if (formChanges[i].method == FORM_CHANGE_BATTLE_MEGA_EVOLUTION_ITEM
            && formChanges[i].param1 != ITEM_NONE)
        {
    ...
    return stones[Random() % count];
}

static bool8 FactorySpeciesHasGigantamax(u16 species)
{
    return DoesSpeciesHaveFormChangeMethod(species, FORM_CHANGE_BATTLE_GIGANTAMAX);
}
...
void CreateFacilityMon(const struct TrainerMon *fmon, u16 level, u8 fixedIV, u32 otID, u32 flags, struct Pokemon *dst)
{
    ...
    u8 gigantamaxFactor = fmon->gigantamaxFactor;
    ...
    for (j = 0; j < MAX_MON_MOVES; j++)
        moves[j] = fmon->moves[j];

    if (flags & FLAG_FRONTIER_MON_FACTORY)
    {
        bool32 bfForms = VarGet(VAR_BATTLE_FACTORY_FORMS) != 0;
        bool32 forceOff = (flags & FLAG_FRONTIER_MON_FACTORY_FORMS_OFF) != 0;

        if (bfForms && !forceOff)
        {
            if (fmon->species == SPECIES_KYOGRE)
                heldItem = ITEM_BLUE_ORB;
            else if (fmon->species == SPECIES_GROUDON)
                heldItem = ITEM_RED_ORB;
            else if (fmon->species == SPECIES_RAYQUAZA)
            {
                bool8 hasMove = FALSE;
                for (j = 0; j < MAX_MON_MOVES; j++)
                {
                    if (moves[j] == MOVE_DRAGON_ASCENT)
                    {
                        hasMove = TRUE;
                        break;
                    }
                ...
                if (!hasMove)
                    moves[0] = MOVE_DRAGON_ASCENT;
            }
        }
        else if (forceOff)
        {
            enum HoldEffect effect = GetItemHoldEffect(heldItem);
            if (effect == HOLD_EFFECT_MEGA_STONE || effect == HOLD_EFFECT_PRIMAL_ORB)
                heldItem = ITEM_NONE;
        }
    }
    ...
    if (flags & FLAG_FRONTIER_MON_FACTORY)
    {
        ...
        if (bfForms && !forceOff)
        {
            u16 megaStone = GetFactoryMegaStoneForSpecies(fmon->species);
            if (megaStone != ITEM_NONE)
                heldItem = megaStone;
            if (gigantamaxFactor == 0 && FactorySpeciesHasGigantamax(fmon->species))
                gigantamaxFactor = 1;
        }
        else if (forceOff)
        {
            if (gigantamaxFactor != 0)
                gigantamaxFactor = 0;
        }
    }
...
static bool8 BattleFactory_CanShowStageScene(void)
{
    ...
    if (GetMonData(&gPlayerParty[0], MON_DATA_SPECIES_OR_EGG) == SPECIES_NONE)
        return FALSE;
...
static u16 BattleFactory_GetMonGraphicsId(struct Pokemon *mon)
{
...
static u8 BattleFactory_SpawnStageMon(u16 graphicsId, u8 virtualObjId, s16 x, s16 y, u8 facing)
{
    ...
    // Use virtual objects to avoid object event limits.
    x -= MAP_OFFSET;
    y -= MAP_OFFSET;
    return CreateVirtualObject(graphicsId, virtualObjId, x, y, 0, facing);
}

static bool8 BattleFactory_GetStageCoordsFromTrainers(s16 *playerX, s16 *playerY, u8 *playerFacing,
                                                      s16 *oppX, s16 *oppY, u8 *oppFacing)
{
    u8 mapNum = gSaveBlock1Ptr->location.mapNum;
    u8 mapGroup = gSaveBlock1Ptr->location.mapGroup;
    u8 playerObjId = GetObjectEventIdByLocalIdAndMap(LOCALID_FACTORY_BATTLE_PLAYER, mapNum, mapGroup);
    u8 oppObjId = GetObjectEventIdByLocalIdAndMap(LOCALID_FACTORY_BATTLE_OPPONENT, mapNum, mapGroup);
    if (playerObjId == OBJECT_EVENTS_COUNT || oppObjId == OBJECT_EVENTS_COUNT)
        return FALSE;

    {
        struct ObjectEvent *playerObj = &gObjectEvents[playerObjId];
        struct ObjectEvent *oppObj = &gObjectEvents[oppObjId];
        *playerFacing = playerObj->facingDirection;
        *oppFacing = oppObj->facingDirection;
        *playerX = playerObj->currentCoords.x + gDirectionToVectors[*playerFacing].x;
        *playerY = playerObj->currentCoords.y + gDirectionToVectors[*playerFacing].y;
        *oppX = oppObj->currentCoords.x + gDirectionToVectors[*oppFacing].x;
        *oppY = oppObj->currentCoords.y + gDirectionToVectors[*oppFacing].y;
    }
    return TRUE;
}

static void BattleFactory_RemoveStageMons(void)
{
    if (sFactoryStagePlayerObjId < MAX_SPRITES && gSprites[sFactoryStagePlayerObjId].inUse)
    {
        DestroySprite(&gSprites[sFactoryStagePlayerObjId]);
        sFactoryStagePlayerObjId = MAX_SPRITES;
    }
    if (sFactoryStageOppObjId < MAX_SPRITES && gSprites[sFactoryStageOppObjId].inUse)
    {
        DestroySprite(&gSprites[sFactoryStageOppObjId]);
        sFactoryStageOppObjId = MAX_SPRITES;
    }
}
...
static void Task_BattleFactoryStageScene(u8 taskId)
{
    ...
    case 0:
        FreezeObjectEvents();
        if (!BattleFactory_CanShowStageScene())
        {
        ...
        {
            s16 playerX, playerY, oppX, oppY;
            u8 playerFacing, oppFacing;
            if (!BattleFactory_GetStageCoordsFromTrainers(&playerX, &playerY, &playerFacing, &oppX, &oppY, &oppFacing))
            {
                UnfreezeObjectEvents();
                ScriptContext_Enable();
                DestroyTask(taskId);
            ...
            sFactoryStagePlayerObjId = BattleFactory_SpawnStageMon(
                BattleFactory_GetMonGraphicsId(&gPlayerParty[0]),
                1, playerX, playerY, playerFacing);

            sFactoryStageOppObjId = BattleFactory_SpawnStageMon(
                BattleFactory_GetMonGraphicsId(&gEnemyParty[0]),
                2, oppX, oppY, oppFacing);
        }

        gTasks[taskId].data[1] = FACTORY_STAGE_PAUSE_FRAMES;
...
void BattleFactory_ShowLeadMonsOnStage(void)
{
    sFactoryStagePlayerObjId = MAX_SPRITES;
    sFactoryStageOppObjId = MAX_SPRITES;
    CreateTask(Task_BattleFactoryStageScene, 1);
}

void BattleFactory_RemoveLeadMonsOnStage(void)
{
    BattleFactory_RemoveStageMons();
}

void BattleFactory_PrepareOpponentParty(void)
{
    gBattleTypeFlags = BATTLE_TYPE_TRAINER | BATTLE_TYPE_FACTORY;
    if (VarGet(VAR_FRONTIER_BATTLE_MODE) == FRONTIER_MODE_DOUBLES)
        gBattleTypeFlags |= BATTLE_TYPE_DOUBLE;
    FillFactoryTrainerParty();
    sFactoryPartyPrepared = TRUE;
}
...
void DoSpecialTrainerBattle(void)
{
    ...
    case SPECIAL_BATTLE_FACTORY:
        ...
        if (VarGet(VAR_FRONTIER_BATTLE_MODE) == FRONTIER_MODE_DOUBLES)
            gBattleTypeFlags |= BATTLE_TYPE_DOUBLE;
        if (!sFactoryPartyPrepared)
            FillFactoryTrainerParty();
        sFactoryPartyPrepared = FALSE;
        CreateTask(Task_StartBattleAfterTransition, 1);
        PlayMapChosenOrBattleBGM(0);
