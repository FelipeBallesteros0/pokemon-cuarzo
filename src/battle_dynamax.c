...
bool32 CanDynamax(u32 battler)
{
    ...
    bool32 bfForms = InBattleFactory() && VarGet(VAR_BATTLE_FACTORY_FORMS) != 0;
    ...
    {
        if (!bfForms && !CheckBagHasItem(ITEM_DYNAMAX_BAND, 1))
            return FALSE;
        if (!bfForms && (B_FLAG_DYNAMAX_BATTLE == 0 || (B_FLAG_DYNAMAX_BATTLE != 0 && !FlagGet(B_FLAG_DYNAMAX_BATTLE))))
            return FALSE;
    }
    ...
    if (!TESTING && !bfForms && (holdEffect == HOLD_EFFECT_Z_CRYSTAL || holdEffect == HOLD_EFFECT_MEGA_STONE))  // tests make this check already
        return FALSE;
