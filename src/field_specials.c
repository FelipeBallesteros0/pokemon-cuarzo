...
#define FOLLOWER_PET_COOLDOWN_STEPS 300
#define FOLLOWER_PET_TRACKED_MON_COUNT 32

struct FollowerPetTracker
{
    u16 species;
    u16 lastStep;
    u32 personality;
...
static EWRAM_DATA struct FollowerPetTracker sFollowerPetTracker[FOLLOWER_PET_TRACKED_MON_COUNT] = {0};
...
static u32 GetFollowerPetTrackerSlot(u16 species, u32 personality, u32 otId)
{
    s32 freeSlot = -1;
    u32 oldestSlot = 0;
    u16 oldestDelta = 0;
    u16 currentStep = gFollowerSteps;
    u32 i;

    for (i = 0; i < FOLLOWER_PET_TRACKED_MON_COUNT; i++)
    {
        if (sFollowerPetTracker[i].used)
        {
            if (sFollowerPetTracker[i].species == species
             && sFollowerPetTracker[i].personality == personality
             && sFollowerPetTracker[i].otId == otId)
                return i;

            if ((u16)(currentStep - sFollowerPetTracker[i].lastStep) >= oldestDelta)
            {
                oldestDelta = (u16)(currentStep - sFollowerPetTracker[i].lastStep);
                oldestSlot = i;
            }
        }
        else if (freeSlot < 0)
...
void Special_FollowerPet(void)
{
    struct Pokemon *mon = GetFollowerPartyMon();
    struct FollowerPetTracker *tracker;
    u16 species = SPECIES_NONE;
    u32 personality;
    u32 otId;
    u32 slot;
    ...
    slot = GetFollowerPetTrackerSlot(species, personality, otId);
    tracker = &sFollowerPetTracker[slot];

    if (tracker->used
     && tracker->species == species
     && tracker->personality == personality
     && tracker->otId == otId
     && (u16)(gFollowerSteps - tracker->lastStep) < FOLLOWER_PET_COOLDOWN_STEPS)
        return;
    ...
    tracker->personality = personality;
    tracker->otId = otId;
    tracker->lastStep = gFollowerSteps;
