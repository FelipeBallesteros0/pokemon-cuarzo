...
EWRAM_DATA static bool8 sFogEntryFadeMode = FALSE;
EWRAM_DATA static bool8 sFogEntryFadeActive = FALSE;
...
void FogHorizontal_Main(void)
{
    ...
    {
    case 0:
        // Stable fog mode: keep horizontal fog in palette-only mode.
        // Underwater still uses the classic sprite layer.
        if (gWeatherPtr->currWeather == WEATHER_UNDERWATER)
            CreateFogHorizontalSprites();
        if (gWeatherPtr->currWeather == WEATHER_FOG_HORIZONTAL)
        {
            if (sFogEntryFadeMode)
            {
                // Field-move Haze: fade in fog smoothly.
                Weather_SetBlendCoeffs(0, 16);
                Weather_SetTargetBlendCoeffs(12, 8, 1);
                gWeatherPtr->fadeScreenCounter = 0;
                sFogEntryFadeMode = FALSE;
                sFogEntryFadeActive = TRUE;
            }
            else
            {
                // Map-load path: keep immediate fog visibility.
    ...
    case 1:
    {
        u16 maxFogCoeff = min((gTimeOfDay + 1) * 4, 12);
        bool8 blendDone;

        if (gWeatherPtr->fadeScreenCounter <= maxFogCoeff)
        {
            ApplyFogPalettesForTransition(gWeatherPtr->fadeScreenCounter);
            gWeatherPtr->fadeScreenCounter++;
        }
        blendDone = Weather_UpdateBlend();
        if (blendDone && gWeatherPtr->fadeScreenCounter > maxFogCoeff)
        {
            gWeatherPtr->weatherGfxLoaded = TRUE;
            sFogEntryFadeActive = FALSE;
            gWeatherPtr->initStep++;
        }
        break;
    }
...
bool8 FogHorizontal_Finish(void)
{
    ...
    case 1:
        if (gWeatherPtr->fadeScreenCounter <= maxFogCoeff)
        {
            ApplyFogPalettesForTransition(maxFogCoeff - gWeatherPtr->fadeScreenCounter);
            gWeatherPtr->fadeScreenCounter++;
        }
        if (Weather_UpdateBlend())
        {
...
void SetFogEntryFadeMode(bool8 enableFade)
{
    sFogEntryFadeMode = enableFade;
}

bool8 IsFogEntryFadeActive(void)
{
    return sFogEntryFadeMode || sFogEntryFadeActive;
}
