...
static void BF_FixDittoMoves(struct Pokemon *mon)
{
...
static const struct TrainerMon *GetFactoryMonList(u8 lvlMode, u16 *outCount)
{
    if (lvlMode == FRONTIER_LVL_TENT)
    {
        if (outCount)
            *outCount = gSlateportBattleTentMonsCount;
        return gSlateportBattleTentMons;
    }

    if (gFacilityTrainerMons != NULL && sFactoryTierCount != 0)
    {
        if (outCount)
            *outCount = sFactoryTierCount;
        return gFacilityTrainerMons;
    }

    if (outCount)
        *outCount = gBattleFrontierMonsCount;
    return gBattleFrontierMons;
}

static bool8 IsFactorySetIdValid(u8 lvlMode, u16 monId)
{
    u16 count = 0;
    const struct TrainerMon *mons = GetFactoryMonList(lvlMode, &count);
    u16 baseId = FactoryGetBaseMonId(monId);

    if (monId == 0xFFFF || baseId >= count)
        return FALSE;

    if (mons[baseId].species == SPECIES_NONE || mons[baseId].species >= NUM_SPECIES)
        return FALSE;

    if (!IsSpeciesEnabled(mons[baseId].species))
        return FALSE;

    if (gSpeciesInfo[mons[baseId].species].speciesName[0] == 0)
        return FALSE;

    if (mons[baseId].species == SPECIES_UNOWN)
        return FALSE;

    if (lvlMode != FRONTIER_LVL_TENT && IsBFBannedSpecies(mons[baseId].species))
        return FALSE;

    if (mons[baseId].species == SPECIES_DITTO)
    {
        bool8 hasTransform = FALSE;
        for (u32 i = 0; i < MAX_MON_MOVES; i++)
        {
            u16 mv = mons[baseId].moves[i];
            if (mv == MOVE_NONE)
                continue;
...
static void BF_ApplyTierFacilityMons(void)
{
    if (gSaveBlock2Ptr->frontier.lvlMode == FRONTIER_LVL_TENT)
    {
        gFacilityTrainerMons = gSlateportBattleTentMons;
        sFactoryTierCount = gSlateportBattleTentMonsCount;
    }
    else
    {
        ...
        sFactoryTierCount = gFactoryTierMonsCount[tier];
    }
}
...
static void GenerateOpponentMons(void)
{
    ...
    while (i != FRONTIER_PARTY_SIZE)
    {
        ...
        u16 baseId = FactoryGetBaseMonId(monId);
        if (!IsFactorySetIdValid(lvlMode, monId))
            continue;
        ...
        for (j = 0; j < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.rentalMons); j++)
        {
            ...
            u16 rentalBaseId = FactoryGetBaseMonId(rentalMonId);
            if (!IsFactorySetIdValid(lvlMode, rentalMonId))
                continue;
            if (mons[baseId].species == mons[rentalBaseId].species)
                break;
        }
        ...
        {
            if (species[k] == mons[baseId].species)
                break;
        }
        if (k != firstMonId + i)
            continue;
        ...
        for (k = firstMonId; k < firstMonId + i; k++)
        {
            if (heldItems[k] != ITEM_NONE && heldItems[k] == mons[baseId].heldItem)
                break;
        }
        if (k != firstMonId + i)
            continue;
        ...
        species[i] = mons[baseId].species;
        heldItems[i] = mons[baseId].heldItem;
...
static void SetRentalsToOpponentParty(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        if (!IsFactorySetIdValid(lvlMode, monId))
        {
            monId = GetFactoryMonId(lvlMode, gSaveBlock2Ptr->frontier.factoryWinStreaks[VarGet(VAR_FRONTIER_BATTLE_MODE)][lvlMode] / FRONTIER_STAGES_PER_CHALLENGE, FALSE);
            baseId = monId;
        }
        gSaveBlock2Ptr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].monId = monId;
        ...
        SetMonData(&gEnemyParty[i], MON_DATA_HELD_ITEM, &mons[baseId].heldItem);
    }
}
...
static void SetPlayerAndOpponentParties(void)
{
    ...
    if (gSpecialVar_0x8005 < 2)
    {
        ...
        for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
        {
            ...
            u16 baseId = FactoryGetBaseMonId(monId);
            u32 facilityFlags = FactoryGetFacilityFlags(monId);

            BF_ApplyTierFacilityMons();
            if (gSaveBlock2Ptr->frontier.lvlMode != FRONTIER_LVL_TENT)
            {
                if (!IsFactorySetIdValid(lvlMode, monId))
                {
                    ...
                    baseId = monId;
                    facilityFlags = FLAG_FRONTIER_MON_FACTORY;
                }
            }

            CreateFacilityMon(&mons[baseId], monLevel, ivs, OT_ID_PLAYER_ID, facilityFlags, &gPlayerParty[i]);
            BF_FixDittoMoves(&gPlayerParty[i]);
        }
    }
    ...
    case 2:
        for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
        {
            ...
            CreateFacilityMon(&mons[baseId], monLevel, ivs, OT_ID_PLAYER_ID, facilityFlags, &gEnemyParty[i]);
            BF_FixDittoMoves(&gEnemyParty[i]);
        }
        break;
    }
}
...
static void GenerateInitialRentalMons(void)
{
    ...
    while (i != PARTY_SIZE)
    {
        ...
        u16 baseId = FactoryGetBaseMonId(monId);
        if (!IsFactorySetIdValid(factoryLvlMode, monId))
            continue;
        ...
        for (j = firstMonId; j < firstMonId + i; j++)
        {
            ...
            if (FactoryGetBaseMonId(existingMonId) == baseId)
                break;
            if (species[j] == mons[baseId].species)
            {
                if (currSpecies == SPECIES_NONE)
                    currSpecies = mons[baseId].species;
                else
                    break;
            }
        }
        ...
        for (j = firstMonId; j < firstMonId + i; j++)
        {
            if (heldItems[j] != ITEM_NONE && heldItems[j] == mons[baseId].heldItem)
            {
                if (mons[baseId].species == currSpecies)
                    currSpecies = SPECIES_NONE;
                break;
            }
        }
...
static void GetOpponentMostCommonMonType(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        u16 baseId = FactoryGetBaseMonId(gFrontierTempParty[i]);
        u32 species = mons[baseId].species;
...
static void GetOpponentBattleStyle(void)
{
    ...
    u8 lvlMode = gSaveBlock2Ptr->frontier.lvlMode;
    u16 monCount = 0;
    const struct TrainerMon *mons = GetFactoryMonList(lvlMode, &monCount);
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        u16 monId = FactoryGetBaseMonId(gFrontierTempParty[i]);
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            u8 battleStyle = GetMoveBattleStyle(mons[monId].moves[j]);
...
static void RestorePlayerPartyHeldItems(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        if (!IsFactorySetIdValid(lvlMode, monId))
            continue;
        SetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM, &mons[baseId].heldItem);
    }
}
...
void FillFactoryBrainParty(void)
{
    ...
    while (i != FRONTIER_PARTY_SIZE)
    {
        ...
        if (gFacilityTrainerMons[baseId].species == SPECIES_UNOWN)
            continue;
        if (monLevel == FRONTIER_MAX_LEVEL_50 && baseId > FRONTIER_MONS_HIGH_TIER)
            continue;

        for (j = 0; j < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.rentalMons); j++)
        {
            if (baseId == FactoryGetBaseMonId(gSaveBlock2Ptr->frontier.rentalMons[j].monId))
                break;
        }
        if (j != (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.rentalMons))
            continue;
        ...
        for (k = 0; k < i; k++)
        {
            if (species[k] == gFacilityTrainerMons[baseId].species)
                break;
        }
        if (k != i)
            continue;
        ...
        for (k = 0; k < i; k++)
        {
            if (heldItems[k] != ITEM_NONE && heldItems[k] == gFacilityTrainerMons[baseId].heldItem)
                break;
        }
        if (k != i)
            continue;

        species[i] = gFacilityTrainerMons[baseId].species;
        heldItems[i] = gFacilityTrainerMons[baseId].heldItem;
        BF_ApplyTierFacilityMons();
        CreateFacilityMon(&gFacilityTrainerMons[baseId],
                monLevel, fixedIV, otId, FactoryGetFacilityFlags(monId),
                &gEnemyParty[i]);
        BF_FixDittoMoves(&gEnemyParty[i]);
        i++;
    }
...
// Valida que el setId sea usable en Battle Factory (evita ??? / moves vacíos)
static u16 GetFactoryMonId(u8 lvlMode, u8 challengeNum, bool8 useBetterRange)
{
    (void)challengeNum;
    (void)useBetterRange;
    ...
    if (sFactoryTierCount == 0 || gFacilityTrainerMons == NULL)
        return 0;

    if (lvlMode != FRONTIER_LVL_TENT)
    {
        for (u32 tries = 0; tries < 4096; tries++)
        {
            u16 monId = Random() % sFactoryTierCount;
            if (IsFactorySetIdValid(lvlMode, monId))
                return monId;
        }

        for (u16 i = 0; i < sFactoryTierCount; i++)
            if (IsFactorySetIdValid(lvlMode, i))
                return i;
        return 0;
    }
...
// Usa el count real (gFacilityTrainerMons es puntero, NO array)
