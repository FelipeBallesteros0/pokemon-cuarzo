...
static void ItemUseOnFieldCB_ElectricBike(u8);
...
static void CB2_OpenBerryPouchFromBag(void);
static bool32 PrepareBerryPouchSelection(void);
static void ItemUseOnFieldCB_Honey(u8 taskId);
static bool32 IsValidLocationForVsSeeker(void);

static const u8 sText_CantDismountBike[] = _("No puedes bajarte de la BICI aquí.{PAUSE_UNTIL_PRESS}");
static const u8 sText_ElectricBikeNoBattery[] = _("No tienes baterías para la Bicicleta Eléctrica.");
...
void ItemUseOutOfBattle_ElectricBike(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    s16 coordsY;
    s16 coordsX;
    ...
    else
    {
        if (Overworld_IsBikingAllowed() && !IsBikingDisallowedByPlayer() && FollowerNPCCanBike())
        {
            ...
            {
                ElectricBike_SetActive(FALSE);
                sItemUseOnFieldCB = ItemUseOnFieldCB_ElectricBike;
                SetUpItemUseOnFieldCallback(taskId);
                return;
            }
            ...
            if (gSaveBlock1Ptr->electricBikeCharge == 0)
            {
                ...
                {
                    DisplayItemMessageOnField(taskId, sText_ElectricBikeNoBattery, Task_CloseCantUseKeyItemMessage);
                    return;
                }
                RemoveBagItem(ITEM_BATERIA_DE_BICICLETA, 1);
                gSaveBlock1Ptr->electricBikeCharge = 100;
            }

            ElectricBike_SetActive(TRUE);
            sItemUseOnFieldCB = ItemUseOnFieldCB_ElectricBike;
            SetUpItemUseOnFieldCallback(taskId);
        }
        else
...
static void ItemUseOnFieldCB_ElectricBike(u8 taskId)
{
    GetOnOffBike(PLAYER_AVATAR_FLAG_MACH_BIKE);
    FollowerNPC_HandleBike();
    ScriptUnfreezeObjectEvents();
...
void ItemUseOutOfBattle_BerryPouch(u8 taskId)
{
    ...
    else if (!HasAtLeastOneBerry())
    {
        DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    }
    else if (gTasks[taskId].tUsingRegisteredKeyItem != TRUE)
    {
        gBagMenu->newScreenCallback = CB2_OpenBerryPouchFromBag;
        Task_FadeAndCloseBagMenu(taskId);
    }
    else
    {
...
static bool32 PrepareBerryPouchSelection(void)
{
    ...
    u16 bagItemId = GetBagItemId(POCKET_BERRIES, *scrollPos + *cursorPos);

    if (bagItemId == ITEM_NONE)
    {
        *scrollPos = 0;
        *cursorPos = 0;
        for (i = 0; i < ITEM_TO_BERRY(LAST_BERRY_INDEX); i++)
        {
            bagItemId = GetBagItemId(POCKET_BERRIES, i);
            if (bagItemId != ITEM_NONE)
            {
                if (i > 4)
                {
    ...
    bagItemId = GetBagItemId(POCKET_BERRIES, *scrollPos + *cursorPos);
    if (bagItemId == ITEM_NONE)
        return FALSE;

    gSpecialVar_ItemId = bagItemId;
    return TRUE;
}
...
void ItemUseOutOfBattle_RechargeableRepel(u8 taskId)
{
    if (gTasks[taskId].tUsingRegisteredKeyItem)
    {
...
void ItemUseOutOfBattle_RepelCharge(u8 taskId)
{
    u16 steps = gSaveBlock1Ptr->rechargeableRepelSteps;
    if (steps >= RECHARGEABLE_REPEL_MAX_STEPS)
    {
        DisplayItemMessage(taskId, FONT_NORMAL, sText_RechargeableRepelFull, CloseItemMessage);
        return;
    }

    steps += RECHARGEABLE_REPEL_CHARGE_STEPS;
    if (steps > RECHARGEABLE_REPEL_MAX_STEPS)
        steps = RECHARGEABLE_REPEL_MAX_STEPS;
    gSaveBlock1Ptr->rechargeableRepelSteps = steps;

    RemoveUsedItem();
    DisplayItemMessage(taskId, FONT_NORMAL, sText_RechargeableRepelCharged, CloseItemMessage);
}
