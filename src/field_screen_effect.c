...
#define METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_LEFT  0x264
...
{
    return metatileId == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_LEFT
        || metatileId == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_RIGHT;
}

static bool8 AlignToCustomPokeballDoor(s16 *x, s16 *y)
{
    s16 baseX = *x;
    s16 baseY = *y;
    s16 bestX = 0;
    ...
    // Search a small area around the player and pick the nearest custom door warp tile.
    for (dy = -2; dy <= 2; dy++)
    {
        for (dx = -2; dx <= 2; dx++)
        {
            s16 xx = baseX + dx;
            s16 yy = baseY + dy;
            u16 metatileId = MapGridGetMetatileIdAt(xx, yy);
            if (IsCustomPokeballDoorMetatile(metatileId))
            {
                s16 score = (dx < 0 ? -dx : dx) + (dy < 0 ? -dy : dy);
                if (score < bestScore)
                {
                    bestScore = score;
                    bestX = xx;
...
static void SetUpWarpExitTask(void)
{
    ...
    if (AlignToCustomPokeballDoor(&doorX, &doorY))
        func = Task_ExitDoor;
    else if (MetatileBehavior_IsDoor(behavior) == TRUE)
        func = Task_ExitDoor;
...
void Task_DoDoorWarp(u8 taskId)
{
    ...
    case DOORWARP_OPEN_DOOR:
        ...
        // Prefer nearby custom 2x2 door metatiles to avoid matching unrelated vanilla doors
        // that share numeric ids in other tilesets (e.g. 0x25D).
        task->tDoorX = *x;
        task->tDoorY = *y;
        task->tDoorTask = -1;
        task->tIsCustomDoor = FALSE;

        if (AlignToCustomPokeballDoor(&task->tDoorX, &task->tDoorY))
        {
            task->tDoorTask = FieldAnimateDoorOpen(task->tDoorX, task->tDoorY);
            task->tIsCustomDoor = TRUE;
        }
    ...
    case DOORWARP_START_WALK_UP:
        ...
        {
            u8 moveAction;
            u16 doorMetatile = MapGridGetMetatileIdAt(task->tDoorX, task->tDoorY);

            if (task->tIsCustomDoor == TRUE
             || doorMetatile == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_LEFT
             || doorMetatile == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_RIGHT)
            {
                // Match the player's current facing so entering doesn't do an up->down bounce.
                moveAction = GetWalkNormalMovementAction(GetPlayerFacingDirection());
            }
            else
            {
                moveAction = GetWalkNormalMovementAction(GetPlayerFacingDirection());
            ...
            ObjectEventSetHeldMovement(&gObjectEvents[playerObjId], moveAction);

            if (PlayerHasFollowerNPC() && !gObjectEvents[followerObjId].invisible)
            {
                u8 newState = DetermineFollowerNPCState(&gObjectEvents[followerObjId], moveAction,
