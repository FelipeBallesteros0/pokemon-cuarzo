...
#define PALNUM_FADE_TEXT 12
#define PALNUM_TEXT      13
...
static inline u16 FactoryGetBaseMonId(u16 monId)
{
    return monId & FACTORY_MONID_MASK;
}

static inline u32 FactoryGetFacilityFlags(u16 monId)
{
    u32 flags = FLAG_FRONTIER_MON_FACTORY;
    if (monId & FACTORY_MONFLAG_FORMS_OFF)
        flags |= FLAG_FRONTIER_MON_FACTORY_FORMS_OFF;
    else if (monId & FACTORY_MONFLAG_FORMS_ON)
        flags |= FLAG_FRONTIER_MON_FACTORY_FORMS_ON;
    return flags;
}

static bool8 FactorySpeciesHasMegaStone(u16 species)
{
...
static bool8 FactorySpeciesHasRayquazaMegaMove(u16 species)
{
    ...
    for (u32 i = 0; formChanges[i].method != FORM_CHANGE_TERMINATOR; i++)
    {
        if (formChanges[i].method == FORM_CHANGE_BATTLE_MEGA_EVOLUTION_MOVE
            && formChanges[i].param1 == MOVE_DRAGON_ASCENT)
            return TRUE;
    }
    return FALSE;
}

static bool8 FactorySpeciesHasPrimal(u16 species)
{
    return species == SPECIES_KYOGRE || species == SPECIES_GROUDON;
}

static bool8 IsValidFactorySetId(const struct TrainerMon *mons, u16 count, u16 monId)
{
    u16 baseId = FactoryGetBaseMonId(monId);

    if (monId == 0xFFFF || baseId >= count || mons == NULL)
        return FALSE;

    const struct TrainerMon *tm = &mons[baseId];
    if (tm->species == SPECIES_NONE || tm->species >= NUM_SPECIES)
        return FALSE;
    if (!IsSpeciesEnabled(tm->species))
        return FALSE;
    if (gSpeciesInfo[tm->species].speciesName[0] == 0)
        return FALSE;
    if (tm->species == SPECIES_UNOWN)
        return FALSE;
    if (IsBFBannedSpecies(tm->species))
        return FALSE;
    if (tm->species == SPECIES_DITTO)
    {
        bool8 hasTransform = FALSE;
        for (u32 i = 0; i < MAX_MON_MOVES; i++)
        {
            u16 mv = tm->moves[i];
            if (mv == MOVE_NONE)
                continue;
            if (mv != MOVE_TRANSFORM)
                return FALSE;
#if defined(MOVES_COUNT)
            if (mv >= MOVES_COUNT)
                return FALSE;
...
static bool8 IsValidFactoryMonData(struct Pokemon *mon)
{
    ...
    {
        if (GetMonData(mon, MON_DATA_MOVE1, NULL) != MOVE_TRANSFORM)
            return FALSE;
    ...
    for (u32 i = 0; i < MAX_MON_MOVES; i++)
    {
        u16 mv = GetMonData(mon, MON_DATA_MOVE1 + i, NULL);
        if (mv == MOVE_NONE)
            return FALSE;
#if BF_NUM_MOVES
        if (mv >= BF_NUM_MOVES)
...
struct FactorySelectScreen
{
    ...
    u8 yesNoCursorPos;
    u8 unused;
    u8 ivs[SELECTABLE_MONS_COUNT];
    u8 othersListWindowId;
    u8 othersListTaskId;
    u16 othersListCount;
    u16 othersListScroll;
    u16 othersListRow;
    struct ListMenuItem *othersListItems;
    u8 (*othersListNames)[40];
    u16 *othersListEntries;
    u16 *othersListIndices;
    s16 othersListLetterStart[27];
    bool8 othersListActive;
    bool8 openOthersOnStart;
    struct Pokemon othersListPreviewMon;
...
static void CB2_InitSelectScreenSimple(void);
static void CB2_SelectScreenSimple(void);
static void VBlankCB_SelectScreenSimple(void);
static void SimpleSelect_Task_HandleInput(u8 taskId);
static void SimpleSelect_BuildList(void);
static void SimpleSelect_UpdateInfoWindow(void);
static void SimpleSelect_ConfirmSelection(void);
static int CompareOthersListIndex(u16 ia, u16 ib);
static void SimpleSelect_FormatListName(u16 listIndex, u8 *dest);
static bool8 SimpleSelect_IsSelected(u16 listIndex);
static void SimpleSelect_ListMoveCursor(s32 itemIndex, bool8 onInit, struct ListMenu *list);
static void SimpleSelect_UpdatePreview(u16 listIndex);
static void SimpleSelect_ShowPreviewMon(struct Pokemon *mon);
static void SimpleSelect_DestroyPreview(void);
static void FixDittoMovesForPreview(struct Pokemon *mon);
...
static void Select_Task_HandleOthersList(u8);
...
static void Select_OpenOthersListMenu(void);
static void Select_CloseOthersListMenu(void);
static void Select_HideBaseSelectSprites(void);
static void Select_ShowBaseSelectSprites(void);
static void Select_HideMonPreviewSprites(void);
static void Select_HideBaseSelectWindows(void);
static void Select_ShowBaseSelectWindows(void);
static void Select_OthersListMoveCursor(s32, bool8, struct ListMenu *);
static void Select_UpdateOthersListPreview(u16);
static void Select_ShowPreviewMonSprite(struct Pokemon *);
static void Select_PrintPreviewSpecies(u16);
static void Select_PrintPreviewMonCategory(u16);
...
static EWRAM_DATA u32 sSelectMenuTilesetSize = 0;
...
static EWRAM_DATA u32 sSwapMenuTilesetSize = 0;

static EWRAM_DATA u16 sSelectMenuPalBuffer[16];
static EWRAM_DATA u16 sSimpleSelectedEntries[FRONTIER_PARTY_SIZE];
static EWRAM_DATA u8 sSimpleSelectedCount;
static EWRAM_DATA u8 sSimpleListWindowId;
static EWRAM_DATA u8 sSimpleInfoWindowId;
static EWRAM_DATA u8 sSimplePanelWindowId;
static EWRAM_DATA u8 sSimpleMonSpriteId;
static EWRAM_DATA u8 sSimpleItemSpriteId;
...
static const struct TrainerMon *sOthersSortMons;
static const u16 *sOthersSortEntries;
...
static const struct BgTemplate sSimple_BgTemplates[] =
{
    {
        .bg = 0,
        .charBaseIndex = 2,
...
static const struct WindowTemplate sSimple_WindowTemplates[] =
{
    [SIMPLE_WIN_INFO] = {
        .bg = 0,
        .tilemapLeft = 1,
...
static const struct WindowTemplate sSelect_OthersListWindowTemplate =
{
    .bg = 0,
    .tilemapLeft = 15,
    .tilemapTop = 2,
...
static const u8 sMenuOptionTextColors[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_LIGHT_GRAY};
static const u8 sSpeciesNameTextColors[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_RED, TEXT_COLOR_TRANSPARENT};
static const u8 sText_BF_List_FormOn[] = _(" (Forma)");
static const u8 sText_BF_List_FormOff[] = _(" (Base)");
static const u8 sText_SimpleSelectChosen[] = _("Elegidos: ");
static const u8 sText_SimpleSelectOf3[] = _("/3");
static const u8 sText_SimpleSelectConfirm[] = _("START: Confirmar");
static const u8 sText_SimpleSelectConfirmShort[] = _(" START: Confirmar");
static const u8 sSimpleInfoTextColors[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_LIGHT_GRAY};
// Match ListMenu text colors (cursorPal/cursorShadowPal) so it renders white like the list.
static const u8 sSimplePanelTextColors[] = {0, 2, 3};
static const u8 sText_SimpleItemLabel[] = _("Objeto: ");
static const u8 sText_SimpleItemNone[] = _("(Ninguno)");
static const u8 sText_SimpleListUnsel[] = _("  ");
static const u8 sText_SimpleListSel[] = _("> ");

#define SIMPLE_ITEM_ICON_TAG  61000
...
void DoBattleFactorySelectScreen(void)
{
    ...
    SetMainCallback2(CB2_InitSelectScreenSimple);
}
...
static void CB2_InitSelectScreen(void)
{
    ...
    case 2:
        ...
        sSelectMenuTilesetBuffer = malloc_and_decompress(gPokemonSelectBgTiles, &sSelectMenuTilesetSize);
        if (sSelectMenuTilesetBuffer == NULL)
            sSelectMenuTilesetSize = 0;
        CpuCopy16(sMonPicBg_Gfx, sSelectMonPicBgTilesetBuffer, sizeof(sMonPicBg_Gfx));
        if (sSelectMenuTilesetSize != 0)
        {
            LoadBgTiles(0, sSelectMenuTilesetBuffer, sSelectMenuTilesetSize, 0);
            LoadBgTiles(1, sSelectMenuTilesetBuffer, sSelectMenuTilesetSize, 0);
        }
        LoadBgTiles(3, sSelectMonPicBgTilesetBuffer, sizeof(sMonPicBg_Gfx), 0);
        CpuFill16(0, sSelectMenuTilemapBuffer, BG_SCREEN_SIZE);
        DecompressDataWithHeaderWram(gPokemonSelectBgTilemap, sSelectMenuTilemapBuffer);
        LoadBgTilemap(0, sSelectMenuTilemapBuffer, BG_SCREEN_SIZE, 0);
        {
            u16 *map = (u16 *)sSelectMenuTilemapBuffer;
            s32 x, y;
            for (y = 11; y <= 16; y++)
            {
                for (x = 0; x < 32; x++)
                    map[y * 32 + x] = map[10 * 32 + x];
            }
        }
        LoadBgTilemap(1, sSelectMenuTilemapBuffer, BG_SCREEN_SIZE, 0);
        DecompressDataWithHeaderWram((const u32 *)gPokemonSelectBgPal, sSelectMenuPalBuffer);
        LoadPalette(sSelectMenuPalBuffer, 0, 2 * PLTT_SIZE_4BPP);
    ...
    case 3:
        ...
        SetBgTilemapBuffer(0, sSelectMenuTilemapBuffer);
        SetBgTilemapBuffer(1, sSelectMenuTilemapBuffer);
        ...
        FillBgTilemapBufferRect(3, 0, 12, 5, 6, 6, 0);
        FillBgTilemapBufferRect(3, 0,  3, 5, 6, 6, 0);
        FillBgTilemapBufferRect(3, 0, 21, 5, 6, 6, 0);
...
#endif
        {
            Select_SetWinRegs(40, 104, 32, 96);
            ShowBg(3);
    ...
    case 6:
        ...
        ClearWindowTilemap(SELECT_WIN_PREVIEW);
        CopyWindowToVram(SELECT_WIN_PREVIEW, COPYWIN_MAP);
        gMain.state++;
        break;
    case 7:
    ...
    case 10:
        ...
        if (!sFactorySelectScreen->fromSummaryScreen)
        {
            ...
            if (sFactorySelectScreen->openOthersOnStart)
            {
                Select_OpenOthersListMenu();
                sFactorySelectScreen->openOthersOnStart = FALSE;
                gTasks[taskId].tState = 0;
                gTasks[taskId].func = Select_Task_HandleOthersList;
            }
        }
        else
        {
...
static void CB2_InitSelectScreenSimple(void)
{
    ...
    case 0:
        ...
        CpuFill32(0, (void *)VRAM, VRAM_SIZE);
        ResetBgsAndClearDma3BusyFlags(0);
        InitBgsFromTemplates(0, sSimple_BgTemplates, ARRAY_COUNT(sSimple_BgTemplates));
        InitWindows(sSimple_WindowTemplates);
        DeactivateAllTextPrinters();
        gMain.state++;
        break;
    case 1:
    ...
    case 3:
        ...
        sSimpleSelectedCount = 0;
        for (u32 i = 0; i < FRONTIER_PARTY_SIZE; i++)
            sSimpleSelectedEntries[i] = 0xFFFF;
        sSimpleListWindowId = SIMPLE_WIN_LIST;
        sSimpleInfoWindowId = SIMPLE_WIN_INFO;
        sSimplePanelWindowId = SIMPLE_WIN_PANEL;
        sSimpleMonSpriteId = SPRITE_NONE;
        sSimpleItemSpriteId = SPRITE_NONE;
        gMain.state++;
        break;
    case 4:
        SimpleSelect_BuildList();
        SimpleSelect_UpdateInfoWindow();
        ShowBg(0);
        ShowBg(1);
        SetVBlankCallback(VBlankCB_SelectScreenSimple);
        BeginNormalPaletteFade(PALETTES_ALL, 0, 16, 0, RGB_BLACK);
        SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_BG0_ON | DISPCNT_BG1_ON | DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
        CreateTask(SimpleSelect_Task_HandleInput, 0);
        SetMainCallback2(CB2_SelectScreenSimple);
        gMain.state = 0;
        break;
    }
}
...
static void SimpleSelect_UpdateInfoWindow(void)
{
    ...
    FillWindowPixelBuffer(sSimpleInfoWindowId, PIXEL_FILL(0));
    PutWindowTilemap(sSimpleInfoWindowId);
    ptr = StringCopy(ptr, sText_SimpleSelectChosen);
    ConvertIntToDecimalStringN(ptr, sSimpleSelectedCount, STR_CONV_MODE_LEFT_ALIGN, 1);
    StringAppend(ptr, sText_SimpleSelectOf3);
    StringAppend(ptr, sText_SimpleSelectConfirmShort);
    AddTextPrinterParameterized3(sSimpleInfoWindowId, FONT_NORMAL, 2, 0, sSimpleInfoTextColors, 0, text);
    CopyWindowToVram(sSimpleInfoWindowId, COPYWIN_FULL);
}

static void SimpleSelect_BuildList(void)
{
    u16 tier = VarGet(VAR_BATTLE_FACTORY_TIER);
    if (tier >= FACTORY_TIER_COUNT)
        tier = FACTORY_TIER_OU;

    gFacilityTrainerMons = gFactoryTierMons[tier];
    sFactorySelectScreen->othersListCount = gFactoryTierMonsCount[tier];
    if (sFactorySelectScreen->othersListCount == 0)
        return;

    u16 baseCount = sFactorySelectScreen->othersListCount;
    u16 entryCount = 0;
    for (u32 i = 0; i < baseCount; i++)
    {
        u16 species = gFacilityTrainerMons[i].species;
        if (FactorySpeciesHasMegaStone(species) || FactorySpeciesHasRayquazaMegaMove(species) || FactorySpeciesHasPrimal(species))
            entryCount += 2;
        else
            entryCount += 1;
    }

    sFactorySelectScreen->othersListCount = entryCount;
    sFactorySelectScreen->othersListItems = AllocZeroed(sizeof(struct ListMenuItem) * entryCount);
    sFactorySelectScreen->othersListNames = AllocZeroed(sizeof(*sFactorySelectScreen->othersListNames) * entryCount);
    sFactorySelectScreen->othersListEntries = AllocZeroed(sizeof(u16) * entryCount);
    sFactorySelectScreen->othersListIndices = AllocZeroed(sizeof(u16) * entryCount);
    if (sFactorySelectScreen->othersListItems == NULL || sFactorySelectScreen->othersListNames == NULL
        || sFactorySelectScreen->othersListEntries == NULL || sFactorySelectScreen->othersListIndices == NULL)
    {
        FREE_AND_SET_NULL(sFactorySelectScreen->othersListItems);
        FREE_AND_SET_NULL(sFactorySelectScreen->othersListNames);
        FREE_AND_SET_NULL(sFactorySelectScreen->othersListEntries);
        FREE_AND_SET_NULL(sFactorySelectScreen->othersListIndices);
        sFactorySelectScreen->othersListActive = FALSE;
        return;
    }

    for (u32 i = 0; i < entryCount; i++)
        sFactorySelectScreen->othersListIndices[i] = i;

    u32 entryIndex = 0;
    for (u32 i = 0; i < baseCount; i++)
    {
        u16 species = gFacilityTrainerMons[i].species;
        bool8 special = FactorySpeciesHasMegaStone(species) || FactorySpeciesHasRayquazaMegaMove(species) || FactorySpeciesHasPrimal(species);
        if (special)
        {
            sFactorySelectScreen->othersListEntries[entryIndex++] = (u16)i | FACTORY_MONFLAG_FORMS_OFF;
            sFactorySelectScreen->othersListEntries[entryIndex++] = (u16)i | FACTORY_MONFLAG_FORMS_ON;
        }
        else
        {
            sFactorySelectScreen->othersListEntries[entryIndex++] = (u16)i;
        }
    }

    sOthersSortMons = gFacilityTrainerMons;
    sOthersSortEntries = sFactorySelectScreen->othersListEntries;
    for (u32 i = 1; i < entryCount; i++)
    {
        u16 key = sFactorySelectScreen->othersListIndices[i];
        s32 j = (s32)i - 1;
        while (j >= 0 && CompareOthersListIndex(sFactorySelectScreen->othersListIndices[j], key) > 0)
        {
            sFactorySelectScreen->othersListIndices[j + 1] = sFactorySelectScreen->othersListIndices[j];
            j--;
        }
        sFactorySelectScreen->othersListIndices[j + 1] = key;
    }

    for (u32 i = 0; i < entryCount; i++)
    {
        u8 *name = sFactorySelectScreen->othersListNames[i];
        SimpleSelect_FormatListName(i, name);
        sFactorySelectScreen->othersListItems[i].name = name;
        sFactorySelectScreen->othersListItems[i].id = i;
    }

    FillWindowPixelBuffer(sSimpleListWindowId, PIXEL_FILL(0));
    PutWindowTilemap(sSimpleListWindowId);
    FillWindowPixelBuffer(sSimplePanelWindowId, PIXEL_FILL(0));
    PutWindowTilemap(sSimplePanelWindowId);

    gMultiuseListMenuTemplate = (struct ListMenuTemplate){0};
    gMultiuseListMenuTemplate.items = sFactorySelectScreen->othersListItems;
    gMultiuseListMenuTemplate.totalItems = sFactorySelectScreen->othersListCount;
    gMultiuseListMenuTemplate.maxShowed = 8;
    gMultiuseListMenuTemplate.windowId = sSimpleListWindowId;
    gMultiuseListMenuTemplate.header_X = 0;
    gMultiuseListMenuTemplate.item_X = 4;
    ...
    gMultiuseListMenuTemplate.moveCursorFunc = SimpleSelect_ListMoveCursor;

    sFactorySelectScreen->othersListActive = TRUE;
    sFactorySelectScreen->othersListTaskId = ListMenuInit(&gMultiuseListMenuTemplate, 0, 0);
    CopyWindowToVram(sSimpleListWindowId, COPYWIN_FULL);
    SimpleSelect_UpdatePreview(0);
}
...
static void SimpleSelect_ConfirmSelection(void)
{
    ...
    u8 ivs = GetFactoryMonFixedIV(challengeNum, FALSE);
    u32 otId = T1_READ_32(gSaveBlock2Ptr->playerTrainerId);
    u16 level = (lvlMode != FRONTIER_LVL_50) ? FRONTIER_MAX_LEVEL_OPEN : FRONTIER_MAX_LEVEL_50;
    u16 tier = VarGet(VAR_BATTLE_FACTORY_TIER);
    if (tier >= FACTORY_TIER_COUNT)
        tier = FACTORY_TIER_OU;
    gFacilityTrainerMons = gFactoryTierMons[tier];
    ...
    for (u32 i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        u16 listIndex = sSimpleSelectedEntries[i];
        u16 entryId = sFactorySelectScreen->othersListIndices[listIndex];
        u16 monId = sFactorySelectScreen->othersListEntries[entryId];
        u16 baseId = FactoryGetBaseMonId(monId);

        sFactorySelectScreen->mons[i].selectedId = i + 1;
        sFactorySelectScreen->mons[i].monId = monId;
        CreateFacilityMon(&gFacilityTrainerMons[baseId],
                level, ivs, otId, FactoryGetFacilityFlags(monId),
                &sFactorySelectScreen->mons[i].monData);
        FixDittoMovesForPreview(&sFactorySelectScreen->mons[i].monData);
    }
    for (u32 i = FRONTIER_PARTY_SIZE; i < SELECTABLE_MONS_COUNT; i++)
...
static void SimpleSelect_Task_HandleInput(u8 taskId)
{
    u16 prevScroll = 0;
    u16 prevRow = 0;
    ListMenuGetScrollAndRow(sFactorySelectScreen->othersListTaskId, &prevScroll, &prevRow);
    s32 input = ListMenu_ProcessInput(sFactorySelectScreen->othersListTaskId);
    u16 newScroll = 0;
    u16 newRow = 0;
    ListMenuGetScrollAndRow(sFactorySelectScreen->othersListTaskId, &newScroll, &newRow);
    ...
    if (gTasks[taskId].data[0] == 0)
    {
        if (input >= 0)
        {
            u8 idx = (u8)input;
            bool8 found = FALSE;
            for (u32 i = 0; i < sSimpleSelectedCount; i++)
            {
                if (sSimpleSelectedEntries[i] == idx)
                {
                    for (u32 j = i; j + 1 < sSimpleSelectedCount; j++)
                        sSimpleSelectedEntries[j] = sSimpleSelectedEntries[j + 1];
                    sSimpleSelectedCount--;
                    found = TRUE;
                    break;
                }
            }
            if (!found)
            {
                if (sSimpleSelectedCount < FRONTIER_PARTY_SIZE)
                {
                    sSimpleSelectedEntries[sSimpleSelectedCount++] = idx;
                }
                else
                {
                    PlaySE(SE_FAILURE);
            ...
            SimpleSelect_FormatListName(idx, sFactorySelectScreen->othersListNames[idx]);
            RedrawListMenu(sFactorySelectScreen->othersListTaskId);
            SimpleSelect_UpdateInfoWindow();
        }
        ...
        if (JOY_NEW(START_BUTTON) && sSimpleSelectedCount == FRONTIER_PARTY_SIZE)
        {
            PlaySE(SE_SELECT);
            SimpleSelect_ConfirmSelection();
    ...
    else
    {
        if (!UpdatePaletteFade())
        {
            ...
            DestroyListMenuTask(sFactorySelectScreen->othersListTaskId, NULL, NULL);
            ClearStdWindowAndFrame(sSimpleListWindowId, TRUE);
            ClearWindowTilemap(sSimpleInfoWindowId);
            ClearWindowTilemap(sSimplePanelWindowId);
            SimpleSelect_DestroyPreview();
            FREE_AND_SET_NULL(sFactorySelectScreen->othersListItems);
            FREE_AND_SET_NULL(sFactorySelectScreen->othersListNames);
            FREE_AND_SET_NULL(sFactorySelectScreen->othersListEntries);
            FREE_AND_SET_NULL(sFactorySelectScreen->othersListIndices);
...
static void SimpleSelect_ListMoveCursor(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
    ...
    if (itemIndex < 0)
        return;
    SimpleSelect_UpdatePreview((u16)itemIndex);
}
...
static bool8 SimpleSelect_IsSelected(u16 listIndex)
{
    for (u32 i = 0; i < sSimpleSelectedCount; i++)
    {
        if (sSimpleSelectedEntries[i] == listIndex)
            return TRUE;
    }
    return FALSE;
}
...
static void FixDittoMovesForPreview(struct Pokemon *mon)
{
    if (GetMonData(mon, MON_DATA_SPECIES, NULL) != SPECIES_DITTO)
        return;
...
static void SimpleSelect_FormatListName(u16 listIndex, u8 *dest)
{
    ...
    u16 monId = sFactorySelectScreen->othersListEntries[entryId];
    u16 baseId = FactoryGetBaseMonId(monId);
    u16 species = gFacilityTrainerMons[baseId].species;
    u8 *ptr = dest;

    if (SimpleSelect_IsSelected(listIndex))
        ptr = StringCopy(ptr, sText_SimpleListSel);
    else
        ptr = StringCopy(ptr, sText_SimpleListUnsel);

    ptr = StringCopy(ptr, GetSpeciesName(species));
    if (monId & FACTORY_MONFLAG_FORMS_ON)
        StringAppend(ptr, sText_BF_List_FormOn);
    else if (monId & FACTORY_MONFLAG_FORMS_OFF)
        StringAppend(ptr, sText_BF_List_FormOff);
}
...
static void SimpleSelect_UpdatePreview(u16 listIndex)
{
    ...
    if (listIndex >= sFactorySelectScreen->othersListCount)
        return;

    u16 entryId = sFactorySelectScreen->othersListIndices[listIndex];
    monId = sFactorySelectScreen->othersListEntries[entryId];
    u16 baseId = FactoryGetBaseMonId(monId);

    if (tier >= FACTORY_TIER_COUNT)
        tier = FACTORY_TIER_OU;
    baseCount = gFactoryTierMonsCount[tier];
    if (!IsValidFactorySetId(gFacilityTrainerMons, baseCount, monId))
        return;
    ...
    CreateFacilityMon(&gFacilityTrainerMons[baseId],
            level, ivs, otId, FactoryGetFacilityFlags(monId),
            &sFactorySelectScreen->othersListPreviewMon);
    FixDittoMovesForPreview(&sFactorySelectScreen->othersListPreviewMon);

    SimpleSelect_ShowPreviewMon(&sFactorySelectScreen->othersListPreviewMon);
}
...
static void SimpleSelect_ShowPreviewMon(struct Pokemon *mon)
{
    ...
    sSimpleMonSpriteId = CreateMonPicSprite(species, isShiny, personality, TRUE, 160, 96, 15, TAG_NONE);
    if (sSimpleMonSpriteId == SPRITE_NONE)
    {
        ResetAllPicSprites();
        sSimpleMonSpriteId = CreateMonPicSprite(species, isShiny, personality, TRUE, 160, 96, 15, TAG_NONE);
    }
    if (sSimpleMonSpriteId != SPRITE_NONE)
    {
        gSprites[sSimpleMonSpriteId].centerToCornerVecX = 0;
        gSprites[sSimpleMonSpriteId].centerToCornerVecY = 0;
    }

    if (item != ITEM_NONE)
    {
        sSimpleItemSpriteId = AddItemIconSprite(SIMPLE_ITEM_ICON_TAG, SIMPLE_ITEM_ICON_TAG, item);
        if (sSimpleItemSpriteId != SPRITE_NONE)
        {
            gSprites[sSimpleItemSpriteId].x = 144;
            gSprites[sSimpleItemSpriteId].y = 112;
        }
    }

    moves[0] = GetMonData(mon, MON_DATA_MOVE1, NULL);
    moves[1] = GetMonData(mon, MON_DATA_MOVE2, NULL);
    moves[2] = GetMonData(mon, MON_DATA_MOVE3, NULL);
    moves[3] = GetMonData(mon, MON_DATA_MOVE4, NULL);

    FillWindowPixelBuffer(sSimplePanelWindowId, PIXEL_FILL(0));

    ptr = text;
    ptr = StringCopy(ptr, sText_SimpleItemLabel);
    AddTextPrinterParameterized3(sSimplePanelWindowId, FONT_NORMAL, 0, 60, sSimplePanelTextColors, 0, text);

    ptr = text;
    if (item != ITEM_NONE)
        StringCopy(ptr, GetItemName(item));
    else
        StringCopy(ptr, sText_SimpleItemNone);
    AddTextPrinterParameterized3(sSimplePanelWindowId, FONT_NORMAL, 0, 72, sSimplePanelTextColors, 0, text);

    for (u32 i = 0; i < 4; i++)
    {
        if (moves[i] != MOVE_NONE)
        {
            AddTextPrinterParameterized3(sSimplePanelWindowId, FONT_NORMAL, 0, 0 + (i * 14),
                sSimplePanelTextColors, 0, GetMoveName(moves[i]));
        }
    }

    CopyWindowToVram(sSimplePanelWindowId, COPYWIN_GFX);
}
...
static void SimpleSelect_DestroyPreview(void)
{
    if (sSimpleMonSpriteId != SPRITE_NONE)
    {
        FreeAndDestroyMonPicSprite(sSimpleMonSpriteId);
        sSimpleMonSpriteId = SPRITE_NONE;
    }
    if (sSimpleItemSpriteId != SPRITE_NONE)
    {
        DestroySprite(&gSprites[sSimpleItemSpriteId]);
        FreeSpriteTilesByTag(SIMPLE_ITEM_ICON_TAG);
        FreeSpritePaletteByTag(SIMPLE_ITEM_ICON_TAG);
        sSimpleItemSpriteId = SPRITE_NONE;
    }
}
...
static void Select_InitMonsData(void)
{
    ...
    sFactorySelectScreen->othersListWindowId = 0xFF;
    sFactorySelectScreen->othersListTaskId = 0xFF;
    sFactorySelectScreen->othersListCount = 0;
    sFactorySelectScreen->othersListScroll = 0;
    sFactorySelectScreen->othersListRow = 0;
    sFactorySelectScreen->othersListItems = NULL;
    sFactorySelectScreen->othersListNames = NULL;
    sFactorySelectScreen->othersListEntries = NULL;
    sFactorySelectScreen->othersListIndices = NULL;
    sFactorySelectScreen->othersListActive = FALSE;
    sFactorySelectScreen->openOthersOnStart = TRUE;
    for (i = 0; i < ARRAY_COUNT(sFactorySelectScreen->othersListLetterStart); i++)
        sFactorySelectScreen->othersListLetterStart[i] = -1;
...
static void Select_Task_HandleMenu(u8 taskId)
{
    ...
    case STATE_MENU_HANDLE_INPUT:
        if (JOY_NEW(A_BUTTON))
        {
            ...
            else if (retVal == SELECT_OPEN_LIST)
            {
                gTasks[taskId].tState = 0;
                gTasks[taskId].func = Select_Task_HandleOthersList;
            }
...
static void CreateFrontierFactorySelectableMons(u8 firstMonId)
{
    ...
    u8 challengeNum = gSaveBlock2Ptr->frontier.factoryWinStreaks[battleMode][lvlMode] / 7;
    u8 rentalRank = 0;
    u16 chosenSpecies[SELECTABLE_MONS_COUNT];
    u16 chosenItems[SELECTABLE_MONS_COUNT];

    tier = VarGet(VAR_BATTLE_FACTORY_TIER);
    if (tier >= FACTORY_TIER_COUNT)
        tier = FACTORY_TIER_OU;
    gFacilityTrainerMons = gFactoryTierMons[tier];
    tierCount = gFactoryTierMonsCount[tier];
    ...
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
    {
        chosenSpecies[i] = SPECIES_NONE;
        chosenItems[i] = ITEM_NONE;
        sFactorySelectScreen->ivs[i + firstMonId] = 0;
    }
    ...
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
    {
        if (i < rentalRank)
            ...
            ivs = GetFactoryMonFixedIV(challengeNum, FALSE);
        sFactorySelectScreen->ivs[i + firstMonId] = ivs;

        for (u32 tries = 0; tries < 512; tries++)
        {
            u16 monId = Random() % tierCount;
            u16 species;
            u16 heldItem;
            bool8 dup = FALSE;

            if (!IsValidFactorySetId(gFacilityTrainerMons, tierCount, monId))
                continue;

            species = gFacilityTrainerMons[monId].species;
            heldItem = gFacilityTrainerMons[monId].heldItem;
            for (u32 k = 0; k < i; k++)
            {
                if (chosenSpecies[k] == species)
                {
                    dup = TRUE;
                    break;
                }
                if (chosenItems[k] != ITEM_NONE && chosenItems[k] == heldItem)
                {
                    dup = TRUE;
                    break;
                }
            ...
            chosenSpecies[i] = species;
            chosenItems[i] = heldItem;
            ...
            FixDittoMovesForPreview(&sFactorySelectScreen->mons[i + firstMonId].monData);

            if (IsValidFactoryMonData(&sFactorySelectScreen->mons[i + firstMonId].monData))
                break;
        }
    }
}
...
static void CreateSlateportTentSelectableMons(u8 firstMonId)
{
    ...
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
    {
        ...
        u16 baseId = FactoryGetBaseMonId(monId);
        sFactorySelectScreen->mons[i + firstMonId].monId = monId;
        CreateFacilityMon(&gFacilityTrainerMons[baseId], level, ivs, otId, 0, &sFactorySelectScreen->mons[i + firstMonId].monData);
        FixDittoMovesForPreview(&sFactorySelectScreen->mons[i + firstMonId].monData);
    }
}
...
static void Select_EnsureValidMonData(u8 slot)
{
    u16 tier;
    u16 tierCount;
    u8 lvlMode;
    ...
    if (sFactorySelectScreen == NULL || slot >= SELECTABLE_MONS_COUNT)
        return;

    if (IsValidFactoryMonData(&sFactorySelectScreen->mons[slot].monData))
        return;
    ...
    gFacilityTrainerMons = gFactoryTierMons[tier];
    tierCount = gFactoryTierMonsCount[tier];
    if (tierCount == 0 || gFacilityTrainerMons == NULL)
        return;
    ...
    otId = T1_READ_32(gSaveBlock2Ptr->playerTrainerId);
    ivs = sFactorySelectScreen->ivs[slot];
    ...
    for (u32 tries = 0; tries < 512; tries++)
    {
        ...
        species = gFacilityTrainerMons[monId].species;
        heldItem = gFacilityTrainerMons[monId].heldItem;
        for (u32 k = 0; k < SELECTABLE_MONS_COUNT; k++)
        {
            if (k == slot)
                continue;
            u16 otherId = FactoryGetBaseMonId(sFactorySelectScreen->mons[k].monId);
            if (otherId >= tierCount)
                continue;
            if (gFacilityTrainerMons[otherId].species == species)
            {
                dup = TRUE;
                break;
            }
            if (heldItem != ITEM_NONE && gFacilityTrainerMons[otherId].heldItem == heldItem)
            {
                dup = TRUE;
                break;
            }
        ...
        sFactorySelectScreen->mons[slot].monId = monId;
        CreateFacilityMon(&gFacilityTrainerMons[monId],
                level, ivs, otId, FactoryGetFacilityFlags(monId),
                &sFactorySelectScreen->mons[slot].monData);
        FixDittoMovesForPreview(&sFactorySelectScreen->mons[slot].monData);
        if (IsValidFactoryMonData(&sFactorySelectScreen->mons[slot].monData))
            break;
    }
}
...
static void Select_PrintMonSpecies(void)
{
    ...
    Select_EnsureValidMonData(monId);
...
static void Select_PrintSelectMonString(void)
{
    ...
    DrawDialogueFrame(SELECT_WIN_INFO, FALSE);
    FillWindowPixelBuffer(SELECT_WIN_INFO, PIXEL_FILL(1));
    ...
    AddTextPrinterParameterized3(SELECT_WIN_INFO, FONT_NORMAL, 2, 5, sMenuOptionTextColors, 0, str);
...
static void Select_PrintCantSelectSameMon(void)
{
    DrawDialogueFrame(SELECT_WIN_INFO, FALSE);
    FillWindowPixelBuffer(SELECT_WIN_INFO, PIXEL_FILL(1));
    AddTextPrinterParameterized3(SELECT_WIN_INFO, FONT_NORMAL, 2, 5, sMenuOptionTextColors, 0, gText_CantSelectSamePkmn);
    CopyWindowToVram(SELECT_WIN_INFO, COPYWIN_FULL);
}
...
static void Select_PrintMenuOptions(void)
{
    ...
    DrawDialogueFrame(SELECT_WIN_OPTIONS, FALSE);
    FillWindowPixelBuffer(SELECT_WIN_OPTIONS, PIXEL_FILL(1));
...
static void Select_PrintYesNoOptions(void)
{
    DrawDialogueFrame(SELECT_WIN_YES_NO, FALSE);
    FillWindowPixelBuffer(SELECT_WIN_YES_NO, PIXEL_FILL(1));
...
static int CompareOthersListIndex(u16 ia, u16 ib)
{
    u16 monIdA = FactoryGetBaseMonId(sOthersSortEntries[ia]);
    u16 monIdB = FactoryGetBaseMonId(sOthersSortEntries[ib]);
    const u8 *nameA = GetSpeciesName(sOthersSortMons[monIdA].species);
    const u8 *nameB = GetSpeciesName(sOthersSortMons[monIdB].species);
    int cmp = StringCompare(nameA, nameB);
    if (cmp != 0)
        return cmp;

    u16 flagsA = sOthersSortEntries[ia] & FACTORY_MONFLAGS_MASK;
    u16 flagsB = sOthersSortEntries[ib] & FACTORY_MONFLAGS_MASK;
    if (flagsA != flagsB)
    {
        u8 rankA = (flagsA == FACTORY_MONFLAG_FORMS_OFF) ? 0 : (flagsA == 0 ? 1 : 2);
        u8 rankB = (flagsB == FACTORY_MONFLAG_FORMS_OFF) ? 0 : (flagsB == 0 ? 1 : 2);
...
static u8 GetLetterIndexFromName(const u8 *name)
{
    u8 c = name[0];
    if (c >= 'A' && c <= 'Z')
        return c - 'A';
...
static void Select_OpenOthersListMenu(void)
{
    if (sFactorySelectScreen->othersListWindowId != 0xFF)
        return;

    Select_HideBaseSelectWindows();
    Select_HideBaseSelectSprites();
    Select_HideMonPreviewSprites();
    HideBg(3);
    LoadBgTilemap(0, sSelectMenuTilemapBuffer, BG_SCREEN_SIZE, 0);
    CopyBgTilemapBufferToVram(0);
    LoadBgTilemap(1, sSelectMenuTilemapBuffer, BG_SCREEN_SIZE, 0);
    ...
    ShowBg(1);
    ShowBg(0);
    SetGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_BG0_ON | DISPCNT_BG1_ON);
    ...
    for (u32 i = 0; i < ARRAY_COUNT(sFactorySelectScreen->othersListLetterStart); i++)
        sFactorySelectScreen->othersListLetterStart[i] = -1;
    ...
    for (u32 i = 0; i < entryCount; i++)
    {
        u16 entryId = sFactorySelectScreen->othersListIndices[i];
        u16 monId = sFactorySelectScreen->othersListEntries[entryId];
        u16 baseId = FactoryGetBaseMonId(monId);
        u16 species = gFacilityTrainerMons[baseId].species;
        u8 *name = sFactorySelectScreen->othersListNames[i];
        ...
        if (monId & FACTORY_MONFLAG_FORMS_ON)
            StringAppend(name, sText_BF_List_FormOn);
        else if (monId & FACTORY_MONFLAG_FORMS_OFF)
            StringAppend(name, sText_BF_List_FormOff);
        sFactorySelectScreen->othersListItems[i].name = name;
        sFactorySelectScreen->othersListItems[i].id = i;

        u8 letter = GetLetterIndexFromName(name);
        if (sFactorySelectScreen->othersListLetterStart[letter] == -1)
            sFactorySelectScreen->othersListLetterStart[letter] = i;
    }

    sFactorySelectScreen->othersListWindowId = AddWindow(&sSelect_OthersListWindowTemplate);
    DrawStdWindowFrame(sFactorySelectScreen->othersListWindowId, FALSE);

    gMultiuseListMenuTemplate = (struct ListMenuTemplate){0};
    gMultiuseListMenuTemplate.items = sFactorySelectScreen->othersListItems;
    gMultiuseListMenuTemplate.totalItems = sFactorySelectScreen->othersListCount;
    gMultiuseListMenuTemplate.maxShowed = 12;
    gMultiuseListMenuTemplate.windowId = sFactorySelectScreen->othersListWindowId;
    gMultiuseListMenuTemplate.header_X = 0;
    gMultiuseListMenuTemplate.item_X = 12;
    gMultiuseListMenuTemplate.cursor_X = 2;
    ...
    gMultiuseListMenuTemplate.fontId = FONT_NORMAL;
    gMultiuseListMenuTemplate.cursorKind = 0;
    gMultiuseListMenuTemplate.moveCursorFunc = Select_OthersListMoveCursor;

    sFactorySelectScreen->othersListActive = TRUE;
    sFactorySelectScreen->othersListTaskId = ListMenuInit(&gMultiuseListMenuTemplate, sFactorySelectScreen->othersListScroll, sFactorySelectScreen->othersListRow);
    CopyWindowToVram(sFactorySelectScreen->othersListWindowId, COPYWIN_FULL);
    (void)Select_UpdateOthersListPreview;
}
...
static void Select_CloseOthersListMenu(void)
{
    ...
    sFactorySelectScreen->othersListActive = FALSE;
    DestroyListMenuTask(sFactorySelectScreen->othersListTaskId, &sFactorySelectScreen->othersListScroll, &sFactorySelectScreen->othersListRow);
    ClearStdWindowAndFrame(sFactorySelectScreen->othersListWindowId, TRUE);
    RemoveWindow(sFactorySelectScreen->othersListWindowId);
    sFactorySelectScreen->othersListWindowId = 0xFF;
    sFactorySelectScreen->othersListTaskId = 0xFF;
    sFactorySelectScreen->othersListCount = 0;
    FREE_AND_SET_NULL(sFactorySelectScreen->othersListItems);
    ...
    Select_ShowBaseSelectSprites();
    Select_ShowBaseSelectWindows();
    Select_ReshowMonSprite();
}

static void Select_HideBaseSelectSprites(void)
{
    for (u32 i = 0; i < SELECTABLE_MONS_COUNT; i++)
        gSprites[sFactorySelectScreen->mons[i].ballSpriteId].invisible = TRUE;
...
static void Select_HideMonPreviewSprites(void)
{
    if (sFactorySelectScreen->monPics[1].monSpriteId != SPRITE_NONE)
    {
        FreeAndDestroyMonPicSprite(sFactorySelectScreen->monPics[1].monSpriteId);
        sFactorySelectScreen->monPics[1].monSpriteId = SPRITE_NONE;
    }

    if (sFactorySelectScreen->monPics[1].bgSpriteId != SPRITE_NONE)
    {
        DestroySprite(&gSprites[sFactorySelectScreen->monPics[1].bgSpriteId]);
        sFactorySelectScreen->monPics[1].bgSpriteId = SPRITE_NONE;
    }
...
{
    for (u32 i = 0; i < SELECTABLE_MONS_COUNT; i++)
        gSprites[sFactorySelectScreen->mons[i].ballSpriteId].invisible = FALSE;
    gSprites[sFactorySelectScreen->cursorSpriteId].invisible = FALSE;
}

static void Select_HideBaseSelectWindows(void)
{
    ClearWindowTilemap(SELECT_WIN_TITLE);
    ClearWindowTilemap(SELECT_WIN_SPECIES);
    ClearWindowTilemap(SELECT_WIN_INFO);
    ClearWindowTilemap(SELECT_WIN_MON_CATEGORY);
    ClearWindowTilemap(SELECT_WIN_PREVIEW);
    CopyWindowToVram(SELECT_WIN_TITLE, COPYWIN_MAP);
    CopyWindowToVram(SELECT_WIN_SPECIES, COPYWIN_MAP);
    CopyWindowToVram(SELECT_WIN_INFO, COPYWIN_MAP);
    CopyWindowToVram(SELECT_WIN_MON_CATEGORY, COPYWIN_MAP);
    CopyWindowToVram(SELECT_WIN_PREVIEW, COPYWIN_MAP);
}
...
static void Select_Task_HandleOthersList(u8 taskId)
{
    if (JOY_NEW(L_BUTTON) || JOY_NEW(R_BUTTON))
    {
        u16 arrayId = 0;
        u8 currLetter;
        u8 targetLetter = 0xFF;
        ListMenuGetCurrentItemArrayId(sFactorySelectScreen->othersListTaskId, &arrayId);

        currLetter = GetLetterIndexFromName(sFactorySelectScreen->othersListItems[arrayId].name);
        if (JOY_NEW(R_BUTTON))
        {
            for (u8 i = currLetter + 1; i < 27; i++)
            {
                if (sFactorySelectScreen->othersListLetterStart[i] >= 0)
                {
                    targetLetter = i;
                    break;
                }
            }
            if (targetLetter == 0xFF)
            {
                for (u8 i = 0; i < 27; i++)
                {
                    if (sFactorySelectScreen->othersListLetterStart[i] >= 0)
        ...
        if (targetLetter != 0xFF)
        {
            u16 targetIndex = sFactorySelectScreen->othersListLetterStart[targetLetter];
            u16 scroll = (targetIndex / gMultiuseListMenuTemplate.maxShowed) * gMultiuseListMenuTemplate.maxShowed;
            u16 row = targetIndex - scroll;
            DestroyListMenuTask(sFactorySelectScreen->othersListTaskId, NULL, NULL);
            sFactorySelectScreen->othersListTaskId = ListMenuInit(&gMultiuseListMenuTemplate, scroll, row);
            CopyWindowToVram(sFactorySelectScreen->othersListWindowId, COPYWIN_FULL);
        }
        return;
    }
    ...
    {
        Select_CloseOthersListMenu();
        Select_ShowPreviewMonSprite(&sFactorySelectScreen->mons[sFactorySelectScreen->cursorPos].monData);
        sFactorySelectScreen->fadeSpeciesNameActive = TRUE;
        gTasks[taskId].tState = 1; // STATE_CHOOSE_MONS_HANDLE_INPUT
        gTasks[taskId].func = Select_Task_HandleChooseMons;
        Select_PrintSelectMonString();
        Select_PrintMonCategory();
        Select_PrintMonSpecies();
    ...
    {
        u8 slot = sFactorySelectScreen->cursorPos;
        u16 entryId = sFactorySelectScreen->othersListIndices[input];
        u16 monId = sFactorySelectScreen->othersListEntries[entryId];
        u16 baseId = FactoryGetBaseMonId(monId);
        u16 tier = VarGet(VAR_BATTLE_FACTORY_TIER);
        u16 baseCount;
        u16 level;
        u32 otId = T1_READ_32(gSaveBlock2Ptr->playerTrainerId);
        u8 ivs = sFactorySelectScreen->ivs[slot];
        ...
        if (!IsValidFactorySetId(gFacilityTrainerMons, baseCount, monId))
            return;
        if (!Select_AreSpeciesValid(monId))
            return;

        sFactorySelectScreen->mons[slot].monId = monId;
        gSaveBlock2Ptr->frontier.rentalMons[slot].monId = monId;
        CreateFacilityMon(&gFacilityTrainerMons[baseId],
                level, ivs, otId, FactoryGetFacilityFlags(monId),
                &sFactorySelectScreen->mons[slot].monData);
        FixDittoMovesForPreview(&sFactorySelectScreen->mons[slot].monData);
        ...
        Select_CloseOthersListMenu();
        Select_ShowPreviewMonSprite(&sFactorySelectScreen->mons[slot].monData);
        Select_PrintMonCategory();
        Select_PrintMonSpecies();
        Select_PrintSelectMonString();
...
static void Select_OthersListMoveCursor(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
    ListMenuDefaultCursorMoveFunc(itemIndex, onInit, list);
    (void)itemIndex;
    (void)list;
...
static void Select_UpdateOthersListPreview(u16 listIndex)
{
    u16 monId;
    u16 level;
    u8 ivs;
    ...
    CreateFacilityMon(&gFacilityTrainerMons[baseId],
            ...
            &sFactorySelectScreen->othersListPreviewMon);
    FixDittoMovesForPreview(&sFactorySelectScreen->othersListPreviewMon);
    Select_ShowPreviewMonSprite(&sFactorySelectScreen->othersListPreviewMon);
    Select_PrintPreviewMonCategory(GetMonData(&sFactorySelectScreen->othersListPreviewMon, MON_DATA_SPECIES, NULL));
    Select_PrintPreviewSpecies(GetMonData(&sFactorySelectScreen->othersListPreviewMon, MON_DATA_SPECIES, NULL));
}

static void Select_ShowPreviewMonSprite(struct Pokemon *mon)
{
    u16 species = GetMonData(mon, MON_DATA_SPECIES, NULL);
    u32 personality = GetMonData(mon, MON_DATA_PERSONALITY, NULL);
    bool8 isShiny = GetMonData(mon, MON_DATA_IS_SHINY, NULL);
    ...
    FreeAndDestroyMonPicSprite(sFactorySelectScreen->monPics[1].monSpriteId);
    sFactorySelectScreen->monPics[1].monSpriteId = CreateMonPicSprite(species, isShiny, personality, TRUE, 16, 32, 15, TAG_NONE);
...
static void Select_PrintPreviewSpecies(u16 species)
{
    u8 x;
...
static void Select_PrintPreviewMonCategory(u16 species)
{
    u8 text[30];
    u8 x;
...
static void Select_CreateMonSprite(void)
{
    ...
    if (sFactorySelectScreen->othersListActive)
        mon = &sFactorySelectScreen->othersListPreviewMon;
    else
        mon = &sFactorySelectScreen->mons[monId].monData;
...
static void Select_ReshowMonSprite(void)
{
    ...
    sFactorySelectScreen->monPics[1].bgSpriteId = CreateSprite(&sSpriteTemplate_Select_MonPicBgAnim, 48, 64, 1);
...
static bool32 Select_AreSpeciesValid(u16 monId)
{
    u8 i, j;
    u16 baseId = FactoryGetBaseMonId(monId);
    u32 species = gFacilityTrainerMons[baseId].species;
    ...
    for (i = 1; i < selectState; i++)
    {
        for (j = 0; j < SELECTABLE_MONS_COUNT; j++)
        {
            ...
            {
                u16 otherId = FactoryGetBaseMonId(sFactorySelectScreen->mons[j].monId);
                if (gFacilityTrainerMons[otherId].species == species)
                    return FALSE;
...
static void CB2_InitSwapScreen(void)
{
    ...
    case 2:
        ...
        FreeAllSpritePalettes();
        ResetAllPicSprites();
        sSwapMenuTilesetBuffer = malloc_and_decompress(gPokemonSelectBgTiles, &sSwapMenuTilesetSize);
        CpuCopy16(sMonPicBg_Gfx, sSwapMonPicBgTilesetBuffer, sizeof(sMonPicBg_Gfx));
        if (sSwapMenuTilesetSize != 0)
            LoadBgTiles(1, sSwapMenuTilesetBuffer, sSwapMenuTilesetSize, 0);
        LoadBgTiles(3, sSwapMonPicBgTilesetBuffer, sizeof(sMonPicBg_Gfx), 0);
        CpuFill16(0, sSwapMenuTilemapBuffer, BG_SCREEN_SIZE);
        DecompressDataWithHeaderWram(gPokemonSelectBgTilemap, sSwapMenuTilemapBuffer);
...
{
    struct Task *task = &gTasks[taskId];
    bool8 isSwapScreen = task->tIsSwapScreen;
    switch (task->tState)
    {
    case 0:
        // Init
        ...
        {
            task->tWinLeft = 40;
            task->tWinRight = DISPLAY_WIDTH - 136;
        }
        task->tWinTop = 64;
        task->tWinBottom = 65;
        SetGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_WIN0_ON);
