...
#define METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL 0x264
#define METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_RIGHT 0x265
...
static const struct DoorGraphics *GetCustomPokeballDoorGraphics(const struct DoorGraphics *gfx, u16 metatileId);
...
static const u16 sDoorAnimPalette_EdificioPokeball[] = INCBIN_U16("graphics/door_anims/edificio_pokeball.gbapal");
...
static const u8 sDoorAnimPalettes_EdificioPokeball[] = {11, 11, 11, 11, 11, 11, 11, 11};
...
static const struct DoorGraphics sDoorAnimGraphicsTable[] =
{
    ...
    {METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL,           DOOR_SOUND_NORMAL,  1, sDoorAnimTiles_EdificioPokeballRight, sDoorAnimPalettes_EdificioPokeball},
    {METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_RIGHT,     DOOR_SOUND_NORMAL,  1, sDoorAnimTiles_EdificioPokeballLeft, sDoorAnimPalettes_EdificioPokeball},
    {},
};
...
static const struct DoorGraphics *GetLinkedCustomPokeballDoor(const struct DoorGraphics *gfx, u32 x, u32 y, u32 *linkedX, u32 *linkedY)
{
    s32 dx;
    u16 linkedMetatile = 0;

    if (!IsCustomPokeballDoorMetatile(gfx->metatileNum))
        return NULL;

    *linkedY = y;

    // Find the other custom door nearby on the same warp row.
    // This avoids hardcoding a fixed horizontal offset.
    for (dx = -3; dx <= 3; dx++)
    {
        u16 candidateMetatile;
        ...
        candidateMetatile = MapGridGetMetatileIdAt((u32)((s32)x + dx), y);
        if (IsCustomPokeballDoorMetatile(candidateMetatile))
        {
            *linkedX = (u32)((s32)x + dx);
            linkedMetatile = candidateMetatile;
            break;
        }
    }

    if (!IsCustomPokeballDoorMetatile(linkedMetatile))
        return NULL;

    // Use the custom door resolver to avoid metatile-id collisions with vanilla entries.
    return GetCustomPokeballDoorGraphics(sDoorAnimGraphicsTable, linkedMetatile);
}
...
static void DrawSingleDoorAnimFrameAt(const struct DoorGraphics *gfx, u32 x, u32 y, u16 tileStart)
{
    ...
    BuildDoorTiles(&tiles[0], tileStart + 0, gfx->palettes);
    DrawDoorMetatileAt(x, y - 1, &tiles[0]);

    BuildDoorTiles(&tiles[0], tileStart + 4, &((const u8 *)gfx->palettes)[4]);
    DrawDoorMetatileAt(x, y, &tiles[0]);
}

static void SortLinkedDoorPairByX(
    const struct DoorGraphics *gfxA, u32 xA, u32 yA,
    const struct DoorGraphics *gfxB, u32 xB, u32 yB,
    const struct DoorGraphics **leftGfx, u32 *leftX, u32 *leftY,
    const struct DoorGraphics **rightGfx, u32 *rightX, u32 *rightY)
{
    *leftGfx = gfxA;
    *leftX = xA;
    *leftY = yA;
...
static void DrawDoor(const struct DoorGraphics *gfx, const struct DoorAnimFrame *frame, u32 x, u32 y)
{
    u32 linkedX = 0;
    u32 linkedY = 0;
    const struct DoorGraphics *linkedGfx = GetLinkedCustomPokeballDoor(gfx, x, y, &linkedX, &linkedY);

    if (frame->offset == 0xFFFF)
    {
        if (linkedGfx != NULL)
        {
            const struct DoorGraphics *leftGfx;
            const struct DoorGraphics *rightGfx;
            u32 leftX, leftY, rightX, rightY;

            SortLinkedDoorPairByX(gfx, x, y, linkedGfx, linkedX, linkedY,
                                  &leftGfx, &leftX, &leftY, &rightGfx, &rightX, &rightY);
            DrawClosedDoorTiles(leftGfx, leftX, leftY);
            DrawClosedDoorTiles(rightGfx, rightX, rightY);
        }
        else
        {
    ...
    else
    {
         ...
         || gfx->tiles == sDoorAnimTiles_EdificioPokeballRight)
        {
            ...
            LoadPalette(sDoorAnimPalette_EdificioPokeball, BG_PLTT_ID(11), PLTT_SIZE_4BPP);
            // Door animation palette is loaded at runtime, so re-apply DNS blend for this slot.
            UpdatePalettesWithTime(1 << 11);
            // In blend-only fog mode, this palette is reloaded each frame, so apply fog tint too.
            if (gWeatherPtr->currWeather == WEATHER_FOG_HORIZONTAL
             || gWeatherPtr->currWeather == WEATHER_FOG_DIAGONAL)
            {
                fogCoeff = min((gTimeOfDay + 1) * 3, 9);
                BlendPalettes(1 << 11, fogCoeff, RGB(28, 31, 28));
            }
        }
        ...
        if (linkedGfx != NULL)
        {
            ...
            // Keep the right-side door on the standard single-door slot.
            // Some map contexts show artifacts when right door uses the secondary slot.
...
static const struct DoorGraphics *GetCustomPokeballDoorGraphics(const struct DoorGraphics *gfx, u16 metatileId)
{
    const struct DoorGraphics *lastMatch = NULL;

    while (gfx->tiles != NULL)
    {
        // Match by metatile id only. Filtering by tile pointer can reject valid custom entries.
        if (gfx->metatileNum == metatileId)
            lastMatch = gfx;
        gfx++;
    }
    return lastMatch;
...
static bool8 IsCustomPokeballDoorMetatile(u16 metatileId)
{
    return metatileId == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL
        || metatileId == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_RIGHT;
}

static const struct DoorGraphics *GetDoorGraphicsAt(const struct DoorGraphics *gfx, u32 *x, u32 *y)
{
    u16 metatileId = MapGridGetMetatileIdAt(*x, *y);
    const struct DoorGraphics *doorGfx;

    // If we're on the top row of this custom doorway, re-anchor to the bottom warp row.
    // This prevents id collisions with vanilla doors that share 0x25D/0x25E labels.
    if ((metatileId == 0x25D || metatileId == 0x25E)
     && IsCustomPokeballDoorMetatile(MapGridGetMetatileIdAt(*x, *y + 1)))
    {
        if (metatileId == 0x25E && *x > 0)
            *x -= 1;
        *y += 1;
        metatileId = MapGridGetMetatileIdAt(*x, *y);
    }

    // Force the custom 2x2 door to use its own graphics entry to avoid id collisions
    // with vanilla door labels in other tilesets.
    if (metatileId == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL
     || metatileId == METATILE_TEST_ISLAND_DOOR_EDIFICIO_POKEBALL_RIGHT)
    {
        return GetCustomPokeballDoorGraphics(gfx, metatileId);
    }

    doorGfx = GetDoorGraphics(gfx, metatileId);

    // For 2x2 doors, support entering through the right warp tile by anchoring to the left tile.
    if (doorGfx == NULL && *x > 0)
    {
        const struct DoorGraphics *leftDoorGfx = GetDoorGraphics(gfx, MapGridGetMetatileIdAt(*x - 1, *y));
        if (leftDoorGfx != NULL && leftDoorGfx->size == 2)
        {
            *x -= 1;
            doorGfx = leftDoorGfx;
        }
